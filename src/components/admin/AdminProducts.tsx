...
export default function AdminProducts() {
  const { categories: categoryOptions, flavours: flavourOptions, occasions: occasionOptions, productTags: tagOptions, filterSections, customSectionDefs } = useStoreConfig();
        ...
        <div className="fixed inset-0 bg-foreground/40 backdrop-blur-sm z-50 flex items-start justify-center pt-10 px-4 overflow-y-auto">
          <div className="bg-card rounded-2xl shadow-elevated w-full max-w-2xl p-6 mb-10">
            ...
            <div className="space-y-4">
              ...
              <DraggableImageList
                images={form.images}
                pendingFiles={pendingAdditionalFiles}
                onReorder={(newImages) => setForm(f => ({ ...f, images: newImages }))}
                onRemoveImage={(i) => removeAdditionalImage(i)}
                onRemovePending={(i) => setPendingAdditionalFiles(f => f.filter((_, idx) => idx !== i))}
                onReorderPending={(newPending) => setPendingAdditionalFiles(newPending)}
                onAddImage={handleAddAdditionalImage}
              />
                <ImageCropper
                  open={showCropper}
                  imageSrc={cropSrc || ""}
