...
import { Plus, X, Trash2, Tag, Layers, Palette, Calendar, Pencil, Upload, Image, Eye, EyeOff, BarChart3, Crop, Mail, Bell, BellOff, Send, CheckCircle2, AlertCircle, MapPin, Phone, Truck, UserCog, Lock, AtSign, RefreshCw, CreditCard, QrCode } from "lucide-react";
...
const RESERVED_CONFIG_TYPES = ["homepage_config", "email_notification", "store_info", "filter_section", "delivery_settings", "active_theme", "payment_config"];
...
export default function AdminSettings() {
  ...
  const [activeSection, setActiveSection] = useState<"config" | "coupons" | "banners" | "notifications" | "storeinfo" | "delivery" | "payment" | "account">("config");
  ...
  const loadAll = async () => {
    ...
    if (configRes.data) {
      ...
      // Load payment config
      const paymentRow = (configRes.data as ConfigItem[]).find(c => c.config_type === "payment_config");
      if (paymentRow) {
        try {
          setPaymentConfig({ ...DEFAULT_PAYMENT_CONFIG, ...JSON.parse(paymentRow.value) });
          setPaymentConfigId(paymentRow.id);
        } catch { /* use defaults */ }
      }
    }
